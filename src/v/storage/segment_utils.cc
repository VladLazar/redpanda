...
ss::future<std::vector<compacted_index_reader>> make_indices_readers(
  ...
  const std::optional<ntp_sanitizer_config>& ntp_sanitizer_config) {
    return ssx::async_transform(
      segments.begin(),
      segments.end(),
      [io_pc, ntp_sanitizer_config](ss::lw_shared_ptr<segment>& seg) {
          const auto path = seg->reader().path().to_compacted_index();
          auto f = ss::now();
          if (seg->has_compaction_index()) {
              f = seg->compaction_index().close();
          }
          return f.then([io_pc, &ntp_sanitizer_config, path]() {
              return make_reader_handle(path, ntp_sanitizer_config)
