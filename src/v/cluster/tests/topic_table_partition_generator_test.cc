...
FIXTURE_TEST(test_happy_path_create, topic_table_fixture) {
    ...
    auto res_1 = table.local().apply(std::move(cmd_1), model::offset(1)).get0();
    auto res_2 = table.local().apply(std::move(cmd_2), model::offset(2)).get0();
    auto res_3 = table.local().apply(std::move(cmd_3), model::offset(3)).get0();
    ...
    do {
        ...
        if (next_batch) {
            ...
            for (auto& p_replicas : *next_batch) {
                vlog(test_log.info, "{}", p_replicas.partition);
