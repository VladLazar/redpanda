...
ss::future<result<T, error_outcome>> abs_client::send_request(
  ...
  const object_key& key) {
    ...
    try {
        ...
        if (err.code() == abs_error_code::_unknown) {
            vlog(
              abs_log.error,
          ...
          || err.code() == abs_error_code::container_not_found) {
        ...
        } else if (err.code() == abs_error_code::server_busy) {
            vlog(
              abs_log.warn,
              "{} response received for container {}",
              err.code_string(),
              bucket);
            ...
            vlog(
              abs_log.trace,
              "Received {} retryable error resopnse from ABS: {}",
              err.code_string(),
              err.message());
